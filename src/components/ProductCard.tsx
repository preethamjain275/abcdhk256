...
export function ProductCard({ product, className, variant = 'default' }: ProductCardProps) {
  ...
  // Use existing image or generate one if missing/broken
  const [imgSrc, setImgSrc] = useState(product.images?.[0] || getProductImage(product.name, product.category));

  useEffect(() => {
    // Reset state when product changes to show skeleton again
    setImageLoaded(false);
    setImgSrc(product.images?.[0] || getProductImage(product.name, product.category));
  }, [product]);
        ...
        <div className="relative aspect-square overflow-hidden bg-muted">
          ...
          {!imageLoaded && (
            <div className="absolute inset-0 z-10 bg-muted animate-pulse flex items-center justify-center">
              <ShoppingBag className="h-8 w-8 text-muted-foreground/20 animate-bounce" />
            </div>
          )}
